...
import { Target, Send, Volume2, VolumeX } from 'lucide-react';
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ conversationId, onMessageSent, onComplete }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="chat-scroll flex-1 overflow-y-auto mb-4 p-4 bg-slate-50/60 rounded-xl border border-slate-100/80 space-y-4">
        {messages.length === 0 ? (
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2 bg-white p-1.5 border border-slate-200 rounded-xl focus-within:ring-2 focus-within:ring-indigo-500/20 focus-within:border-indigo-500 transition-all">
          ...
          <input
            ...
            placeholder={isListening ? 'Listening closely...' : 'Ask about risk, index funds, timelines...'}
          ...
          <button
            ...
            type="button"
            aria-label={autoSpeak ? 'Mute assistant voice' : 'Unmute assistant voice'}
            className={'p-2 rounded-lg text-xs font-semibold transition-all flex items-center gap-1 ' +
              (autoSpeak
          ...
          <button
            ...
            className="px-4 py-2 bg-slate-900 text-white text-sm font-medium rounded-lg hover:bg-slate-800 active:bg-slate-950 transition-colors disabled:opacity-30 disabled:pointer-events-none flex items-center gap-1.5"
          >
            <Send size={16} />
